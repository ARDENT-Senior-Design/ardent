...
namespace ardent_model
{
    ...
    class Transmission
    {
        public:
            ...
            virtual void propagatePosition(std::vector<ardent_hardware_interface::Actuator*>&,
                                                    std::vector<ardent_model::JointState*>&) =0;

            // Uses the joint position to fill out the actuator encoder
            virtual void propagatePositionBackwards(std::vector<ardent_model::JointState*>&,
                                         std::vector<ardent_hardware_interface::Actuator*>&) =0;
            
            // Uses the commanded joint efforts to fill out commanded motor currents
            virtual void propagateEffort(std::vector<ardent_model::JointState*>&,
                                std::vector<ardent_hardware_interface::Actuator*>&) =0;
